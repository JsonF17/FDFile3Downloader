...
@interface TDFile3Download () <NSURLSessionDataDelegate>
@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) NSOperationQueue *sessionDelegateQueue;
@property (nonatomic, strong) NSURLSessionConfiguration *sessionConfiguration;
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, NSError *> *errorDictionary;
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, id<TDFile3DownloadTaskDelegate>> *delegateDictionary;
@property (nonatomic) UIBackgroundTaskIdentifier backgroundTask;
@end

@implementation TDFile3Download

+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        M3U8_queue = dispatch_queue_create("queue.TDFile3Download", DISPATCH_QUEUE_CONCURRENT);
    });
}

+ (instancetype)shared {
    static TDFile3Download *obj = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        obj = [[self alloc] init];
    });
...
- (nullable NSURLSessionTask *)downloadWithRequest:(NSURLRequest *)requestParam priority:(float)priority delegate:(id<TDFile3DownloadTaskDelegate>)delegate {
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)task didReceiveResponse:(__kindof NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    id<TDFile3DownloadTaskDelegate> delegate = [self _delegateForTask:task];
    if ( delegate != nil ) {
...
- (void)_setDelegate:(nullable id<TDFile3DownloadTaskDelegate>)delegate forTask:(NSURLSessionTask *)task {
    dispatch_barrier_sync(M3U8_queue, ^{
...
- (nullable id<TDFile3DownloadTaskDelegate>)_delegateForTask:(NSURLSessionTask *)task {
    __block id<TDFile3DownloadTaskDelegate> delegate = nil;
