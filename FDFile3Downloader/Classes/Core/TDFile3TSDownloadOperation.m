...
@interface TDFile3TSDownloadOperation () {
    BOOL _isCancelled;
}
...
@implementation TDFile3TSDownloadOperation
...
- (void)start {
    @synchronized (self) {
        ...
        _downloadTask = [SJDownloadDataTask downloadWithURLStr:self.url toPath:[NSURL fileURLWithPath:self.path] append:YES response:^BOOL(SJDownloadDataTask * _Nonnull dataTask, NSURLResponse *response) {
            ...
            if ( TDFile3Configuration.shared.allowDownloads != nil ) {
                allows = TDFile3Configuration.shared.allowDownloads(response);
            }
