...
+ (nullable instancetype)fileParserWithURL:(NSString *)url saveKeyToFolder:(NSString *)folder error:(NSError **)error {
    ...
    do {
        ...
        NSData *data = [TDFile3Data dataWithContentsOfRequest:curr networkTaskPriority:1 error:&downloadError willPerformHTTPRedirection:^(NSHTTPURLResponse * _Nonnull response, NSURLRequest * _Nonnull newRequest) {
            curr = newRequest;
        }];
    ...
    TDFile3FileParser *parser = TDFile3FileParser.alloc.init;
    parser.url = url;
...
+ (nullable instancetype)fileParserWithContentsOfFile:(NSString *)path error:(NSError **)error {
    ...
    TDFile3FileParser *fileParser = TDFile3FileParser.alloc.init;
